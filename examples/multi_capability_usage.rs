...
async fn main() -> anyhow::Result<()> {
    ...
    println!("  💬 Text Generation:  {}", if has_text_gen { "✓" } else { "✗" });
    println!("  🔢 Embeddings:       {}", if has_embedding { "✓" } else { "✗" });
    println!("  🎤 Transcription:    {}", if has_transcription { "✓" } else { "✗" });
    println!();
    ...
    if has_embedding {
        ...
        match provider.embed_texts(texts.clone()).await {
            Ok(embeddings) => {
                ...
                if embeddings.len() >= 2 {
                    let similarity = cosine_similarity(&embeddings[0].vector, &embeddings[1].vector);
    ...
    println!("\n\n📊 Summary");
    println!("───────────");
    let capabilities_count = [has_text_gen, has_embedding, has_transcription].iter().filter(|&&x| x).count();
    println!("✓ Successfully demonstrated {} capabilities", capabilities_count);
