...
pub async fn create_sqlite_tools(connection_string: &str) -> ZdkResult<Vec<Arc<dyn Tool>>> {
...
pub async fn create_sqlite_tools_with_config(
    ...
    let pool = SqlitePoolOptions::new()
        ...
        .await
        .map_err(|e| ZdkError::Other(anyhow::anyhow!("Failed to connect to SQLite: {}", e)))?;
...
fn create_list_tables_tool(pool: Pool<Sqlite>) -> ZdkResult<FunctionTool> {
...
fn create_describe_table_tool(pool: Pool<Sqlite>) -> ZdkResult<FunctionTool> {
...
fn create_query_tool(pool: Pool<Sqlite>, config: DatabaseToolConfig) -> ZdkResult<FunctionTool> {
    let schema = ToolSchema::new()
...
fn create_execute_tool(pool: Pool<Sqlite>, config: DatabaseToolConfig) -> ZdkResult<FunctionTool> {
    let schema = ToolSchema::new()
