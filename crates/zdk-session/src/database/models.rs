...
impl EventRow {
    /// Convert from zdk_core::Event to EventRow
    pub fn from_event(
        event: &zdk_core::Event,
        app_name: &str,
        user_id: &str,
        session_id: &str,
    ...
    /// Convert from EventRow to zdk_core::Event
    pub fn to_event(&self) -> Result<zdk_core::Event, serde_json::Error> {
        let actions: zdk_core::EventActions = serde_json::from_str(&self.actions)?;
