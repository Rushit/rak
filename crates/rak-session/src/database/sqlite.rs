...
use rak_core::{Error as RakError, Event, Result as RakResult};
use anyhow::anyhow;
...
impl SqliteSessionService {
    ...
    pub async fn new(database_url: &str) -> Result<Self, sqlx::Error> {
        ...
        // Run migrations
        super::migrations::run_sqlite_migrations(&pool).await?;
        super::migrations::create_events_index_sqlite(&pool).await?;
    ...
    async fn get_or_create_app_state(
        ...
        let row: Option<AppStateRow> =
            sqlx::query_as("SELECT * FROM app_states WHERE app_name = ?")
                .bind(app_name)
        ...
        if let Some(row) = row {
            ...
            sqlx::query("INSERT OR IGNORE INTO app_states (app_name, state) VALUES (?, ?)")
                .bind(app_name)
                .bind("{}")
                .execute(&self.pool)
                .await?;
    ...
    async fn get_or_create_user_state(
        ...
        let row: Option<UserStateRow> =
            sqlx::query_as("SELECT * FROM user_states WHERE app_name = ? AND user_id = ?")
                .bind(app_name)
                .bind(user_id)
                .fetch_optional(&self.pool)
                .await?;
        ...
        if let Some(row) = row {
            ...
            sqlx::query(
                "INSERT OR IGNORE INTO user_states (app_name, user_id, state) VALUES (?, ?, ?)",
            )
            .bind(app_name)
            .bind(user_id)
            .bind("{}")
...
impl SessionService for SqliteSessionService {
    ...
    async fn create(&self, req: &CreateRequest) -> RakResult<Arc<dyn Session>> {
        ...
        let state_json = "{}";
        sqlx::query(
            "INSERT OR IGNORE INTO sessions (app_name, user_id, id, state) VALUES (?, ?, ?, ?)",
        )
        .bind(&req.app_name)
        .bind(&req.user_id)
        .bind(&session_id)
