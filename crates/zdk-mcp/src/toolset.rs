...
impl Toolset for McpToolset {
    ...
    async fn get_tools(&self, _ctx: &dyn InvocationContext) -> Result<Vec<Arc<dyn Tool>>> {
        ...
        let client = client_guard
            .as_ref()
            .ok_or_else(|| zdk_core::Error::Other(anyhow::anyhow!("MCP client not initialized")))?;

        // List tools from MCP server
        let mcp_tools = client
            .list_tools()
            .await
            .map_err(|e| zdk_core::Error::Other(anyhow::anyhow!("Failed to list tools: {}", e)))?;
...
impl McpToolsetBuilder {
    ...
    pub fn build(self) -> Result<McpToolset> {
        let name = self
            .name
            .ok_or_else(|| zdk_core::Error::Other(anyhow::anyhow!("Toolset name is required")))?;
        let connection_params = self.connection_params.ok_or_else(|| {
            zdk_core::Error::Other(anyhow::anyhow!("Connection parameters are required"))
        })?;
