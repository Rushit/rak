...
mod tests {
    use super::*;
    use rak_core::{Agent, Content, LLMRequest, LLMResponse, Part, Result, LLM};
    ...
    impl Agent for MockAgent {
        ...
        async fn run(
            &self,
            ctx: Arc<dyn rak_core::InvocationContext>,
        ) -> Box<dyn Stream<Item = Result<rak_core::Event>> + Send + Unpin> {
            ...
            Box::new(Box::pin(stream! {
                ...
                while let Some(response) = llm_stream.next().await {
                    ...
                    let mut event = rak_core::Event::new(invocation_id.clone(), name.clone());
