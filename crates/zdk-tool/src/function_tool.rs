...
impl FunctionToolBuilder {
    ...
    pub fn build(self) -> Result<FunctionTool> {
        Ok(FunctionTool {
            name: self
                .name
                .ok_or_else(|| zdk_core::Error::Other(anyhow::anyhow!("Tool name is required")))?,
            description: self.description.ok_or_else(|| {
                zdk_core::Error::Other(anyhow::anyhow!("Tool description is required"))
            })?,
            schema: self.schema.unwrap_or(Value::Null),
            is_long_running: self.is_long_running,
            execute_fn: self.execute_fn.ok_or_else(|| {
                zdk_core::Error::Other(anyhow::anyhow!("Tool execute function is required"))
            })?,
        })
    }
}
