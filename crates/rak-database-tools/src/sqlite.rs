...
use rak_core::{Error as RakError, Result as RakResult, Tool, ToolContext, ToolResponse};
...
pub async fn create_sqlite_tools(connection_string: &str) -> RakResult<Vec<Arc<dyn Tool>>> {
...
pub async fn create_sqlite_tools_with_config(
    ...
    let pool = SqlitePoolOptions::new()
        ...
        .await
        .map_err(|e| RakError::Other(anyhow::anyhow!("Failed to connect to SQLite: {}", e)))?;
...
fn create_list_tables_tool(pool: Pool<Sqlite>) -> RakResult<FunctionTool> {
...
fn create_describe_table_tool(pool: Pool<Sqlite>) -> RakResult<FunctionTool> {
...
fn create_query_tool(pool: Pool<Sqlite>, config: DatabaseToolConfig) -> RakResult<FunctionTool> {
    let schema = ToolSchema::new()
...
fn create_execute_tool(pool: Pool<Sqlite>, config: DatabaseToolConfig) -> RakResult<FunctionTool> {
    let schema = ToolSchema::new()
