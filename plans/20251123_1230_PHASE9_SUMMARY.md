...
### 1. **New Profiling Module** (`rak-telemetry/profiling/`)
...
- ✅ Traces (spans showing execution flow)
- ✅ Metrics (counters, histograms)

**After (Phase 9 with profiling)**:
- ✅ Traces
- ✅ Metrics
- 🆕 **CPU Profiles** (where CPU time goes)
- 🆕 **Memory Profiles** (allocation patterns)
- 🆕 **Resource Attribution** (cost per agent/session)
...
└── profiling/           ← NEW MODULE
    ...
    ├── attribution.rs   ← Resource tracking
    ├── exporters.rs     ← pprof exporters
...
|------------|-------------------|----------------|
| **"Which agent is slow?"** | ✅ Via trace duration | ✅ Same |
| **"Why is it slow?"** | ❌ Unknown | ✅ See CPU flamegraph |
| **"Which function is hot?"** | ❌ | ✅ pprof shows exact functions |
| **"Is there a memory leak?"** | ❌ | ✅ Memory timeline shows growth |
| **"How much RAM per agent?"** | ❌ | ✅ Attribution per invocation |
| **"What's the cost?"** | ⚠️ Duration only | ✅ CPU + Memory costs |
...
3. 🤷 **Can't tell why the tool is slow**

**With Profiling**:
1. Find slow trace in Jaeger: "agent.run took 8s"
2. See it called `tool.execute` which took 6s
3. Click "View Profile" → jump to Pyroscope
4. 🎯 **See flamegraph showing `regex::match` taking 60% of CPU**
5. 🎯 **Fix the regex, optimize, redeploy**
6. ✅ **Verify P95 latency drops from 8s → 1.5s**
...
**Without Profiling**:
- Traces → Jaeger, Tempo, GCP Cloud Trace
- Metrics → Prometheus

**With Profiling**:
- Traces → Jaeger, Tempo, GCP Cloud Trace
- Metrics → Prometheus
- **Profiles → Pyroscope, GCP Cloud Profiler, Grafana Pyre** ← NEW
...
✅ **If**: You need observability quickly  
✅ **If**: You're okay with "what happened" without "why slow"  
❌ **If**: You need to optimize performance

### Start with Full (with profiling)?
✅ **If**: Performance is critical  
✅ **If**: You need cost attribution  
✅ **If**: You're optimizing production systems  
⚠️ **If**: You can accept 5-10% overhead
...
1. **20251123_1200_PHASE9_ADVANCED_OBSERVABILITY_DESIGN.md**  
   → Complete Phase 9 design (base observability)

2. **20251123_1210_PROFILING_DESIGN.md**  
   → CPU & memory profiling extension

3. **20251123_1220_COMPLETE_OBSERVABILITY_SYSTEM.md**  
   → Full architecture with everything integrated
...
- ✅ **+1 new module** (`profiling/`)
- ✅ **+6 new files** (~1500 lines of code)
- ✅ **+4 new dependencies** (pprof, dashmap, etc.)
- ✅ **+1 new backend** (Pyroscope or GCP Cloud Profiler)
- ✅ **+34 hours** of implementation time
- ✅ **+~50% data volume** (still small: ~17 MB/hour)
- ✅ **+3-5% performance overhead** (acceptable)

**Benefits**:
- 🎯 **10x faster** root cause analysis
- 🎯 **Exact function-level** bottleneck identification
- 🎯 **Memory leak** detection
- 🎯 **Cost attribution** per agent/session
- 🎯 **Optimization guidance** with flamegraphs
